...
def cargar_queries(archivo=None):
    if archivo is None:
        # Construir la ruta relativa al archivo queries.sql
        current_dir = os.path.dirname(os.path.abspath(__file__))
        archivo = os.path.join(current_dir, 'queries', 'queries.sql')
...
def scd_type1(df_actual: pd.DataFrame, 
    ...
    # Asegurarse de que las columnas para comparar estén en ambos DataFrames
    for col in columnas_para_comparar:
        if col not in df_nuevo_work.columns:
