WITH 
-- CTE 1: ANTERIORES - Optimizado con filtro directo
ANTERIORES AS (
...
-- CTE 3: Clasificación SUPER OPTIMIZADA con JOINs
CLASIFICACION_ARTICULOS AS (
    SELECT 
        ACM."ItemCode",
        CASE WHEN CP."U_SYS_CODA" IS NOT NULL THEN 'CON_PROMEDIO' ELSE 'SIN_PROMEDIO' END AS "TipoArticulo",
        CASE WHEN ANT."U_SYS_CODA" IS NOT NULL THEN 'ANTERIOR' ELSE 'MODERNO' END AS "TipoFecha"
    FROM ARTICULOS_CON_MOVIMIENTO ACM
    LEFT JOIN (SELECT DISTINCT "U_SYS_CODA" FROM "MOBO_PRODUCTIVO"."@SYS_COSTOPROMEDIO") CP ON (CP."U_SYS_CODA" = ACM."ItemCode")
    LEFT JOIN ANTERIORES ANT ON (ANT."U_SYS_CODA" = ACM."ItemCode")
),

-- CTE 4: TABLA1+TABLA2 FUSIONADAS
...
-- CTE 5: Solo las últimas transacciones
...
-- CTE 6: TABLA3+TABLA4 FUSIONADAS - ¡NUEVA OPTIMIZACIÓN!
...
-- CTE 7: Solo los últimos registros de costo promedio
...
-- CTE 8: BASE_ARTICULOS - Sin cambios
BASE_ARTICULOS AS (
    SELECT 
        O."ItemCode",
        O."ItemName",
...
-- CTE 9: CALCULOS_BASE - FILTRADO por artículos activos
CALCULOS_BASE AS (
    SELECT 
        CP."U_SYS_CODA",
        CASE 
        ...
        END AS "ValorCalc"
    FROM "MOBO_PRODUCTIVO"."@SYS_COSTOPROMEDIO" CP
    INNER JOIN ARTICULOS_CON_MOVIMIENTO ACM ON (CP."U_SYS_CODA" = ACM."ItemCode")
    WHERE CP."U_SYS_PROC" = 'Y' 
      AND CP."U_SYS_FECH" <= '01.07.2025'
),

-- CTE 10: COSTOS_OINM - Usa TABLA2_OPTIMIZADA
COSTOS_OINM AS (
    SELECT 
...
-- CTE 11: COSTOS_PROMEDIO - Usa TABLA4_OPTIMIZADA
COSTOS_PROMEDIO AS (
    SELECT 
...
-- CTE 12: ARTICULOS_COSTO - Sin cambios
ARTICULOS_COSTO AS (
...
-- CTE 13: ARTICULO_COSTOVENTAS - Sin cambios
